...
class politica_legendaria(Policy):
    ...
    #la idea es codificar en macro_actions estrategias, como jugar la primera en el centro, y tomar
    # la tercerda fila del centro
    #intentar sugerir el patron odd/even row, sugiriendo o prohibiendo jugadas en columnas para explorar con trials
    #basicamente reducir el espacio de busqueda
    def macro_actions(self,board: np.ndarray) -> (int,List[int], List[int]):
        col=None
        reccomended=[]
        banned=[]
        if np.count_nonzero (board) ==0:
            col=3
            return col,reccomended,banned
        if self.get_heights(board)[3]==1:
            banned.append(3)
        elif self.get_heights(board)[3]==2:
            reccomended.append(3)
        #odd/even
        heights=self.get_heights(board)
        legal=self.get_available_cols(board)
        for c in legal:
            fila= 5- heights[c]
            if self.get_current_player(board)==-1 and fila %2 ==1:
                reccomended.append(c)
            if self.get_current_player(board)==1 and fila%2 ==0:
                reccomended.append(c)
        return col,reccomended,banned
    ...
    def act(self, s: np.ndarray) -> int:
        ...
        if col is not None:
            return int(col)
        #macro
        col,preferir,evitar=self.macro_actions(s)
        if col is not None:
            return int(col)
        ...
        if preferir:
            legal= [c for c in preferir if c in legal]
        #hacer trials con todas las columans menos las que sugiere macro evitar
        if evitar:
            legal=[c for c in legal if c not in evitar]
        if not legal:
            legal=self.get_available_cols(s)
