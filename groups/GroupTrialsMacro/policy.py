...
class politica_rara(Policy):
    ...
    def act(self, s: np.ndarray) -> int:
        ...
        def trial(t:int) -> dict:
            ...
            for i in range(t):
                ...
                for col in  get_available_cols(test_s) :
                    ...
                    macro_col, recomendado,baneado= macro_actions(temp_board)
                    sequence=[col]
                    if recomendado:
                        sequence.append(rng.choice(recomendado))
                    for c in sequence:
                        if get_next_row(c,temp_board) is not None:
                            play(c, yo, temp_board) #C: prueba jugar cada columna como primer movimiento
                    winner = None
                    tries = 0
                    while 0 in temp_board and winner is None: #C: Aqui vuelve a el comportamiento que ya tenías
                        ...
                        if col_me is not None and col_me in legal:
                            ...
                            col, reco,ban = macro_actions(temp_board)
                            if reco:
                                play(rng.choice(reco),yo,temp_board)
                            else:
        ...
        def macro_actions(board: np.ndarray) -> (int,List[int], List[int]):
            col=None
            reccomended=[]
            banned=[]
            ...
            if get_heights(board)[3]==1:
                banned.append(3)
            elif get_heights(board)[3]==2:
            ...
            for c in legal:
                fila= 5- heights[c]
                if get_current_player(board)==-1 and fila %2 ==1:
                    reccomended.append(c)
                if get_current_player(board)==1 and fila%2 ==0:
                    reccomended.append(c)
            return col,reccomended,banned
